...
class IndexController
{
	...
	{
		// 文件设置浏览器缓存
		header("Cache-Control: max-age=" . config('cache_expire_time'));
		header("Pragma: cache");
		header("Expires: " . gmdate("D, d M Y H:i:s", time() + config('cache_expire_time')) . " GMT");

		// $item = $this->items[$this->name];
		$item = cdn::proxy($this->items[$this->name]);
		if ($item['folder']) { //是文件夹
	...
	function thumbnail($item)
	{
		...
		return cdn::proxy($item)['thumb'] . "width={$width}&height={$height}";
	}
